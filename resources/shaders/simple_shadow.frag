...
void main()
{
  ...
  float shadow = 0.0f;
  if (shadowTexCoord.x >= 0.0001f && shadowTexCoord.y >= 0.0001f shadowTexCoord.x <= 0.9999f && shadowTexCoord.y <= 0.9999f) {
    if (posLightSpaceNDC.z < textureLod(shadowMap, shadowTexCoord, 0).x + 0.001f) {
        ...
        shadow = 1.0f - clamp((angle - Params.inLightAngle) / (Params.outLightAngle - Params.inLightAngle), 0.0f, 1.0f);
    }
  }
