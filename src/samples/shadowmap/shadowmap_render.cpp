...
void SimpleShadowmapRender::loadShaders()
{
  ...
  etna::create_program("tonemap", { VK_GRAPHICS_BASIC_ROOT "/resources/shaders/tone_map.comp.spv" });
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  //
  {
    ...
    etna::RenderTargetState renderTargets(a_cmdBuff, { 0, 0, m_width, m_height }, { { .image = mainView.get(), .view = mainView.getView({}) } }, { .image = mainViewDepth.get(), .view = mainViewDepth.getView({}) });
  ...
  {
    {
      etna::set_state(a_cmdBuff, mainView.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
      etna::set_state(a_cmdBuff, mainViewTonemapped.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderWrite, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
      ...
      vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_tonemappingPipeline.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);

      vkCmdDispatch(a_cmdBuff, m_width, m_height, 1);
    }

    {
      etna::set_state(a_cmdBuff, mainViewTonemapped.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
      etna::flush_barriers(a_cmdBuff);

      auto quadRGB8Info = etna::get_shader_program("quad_rgb8");

      auto set = etna::create_descriptor_set(quadRGB8Info.getDescriptorLayoutId(0), a_cmdBuff, { etna::Binding{ 0, mainViewTonemapped.genBinding(defaultSampler.get(), vk::ImageLayout::eGeneral) } });
      ...
      vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_quadRGB8Pipeline.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);
  ...
  else
  {
    ...
    vk::ImageBlit region = {
      ...
      .srcOffsets     = { { vk::Offset3D{ 0, 0, 0 }, vk::Offset3D{ static_cast<int32_t>(m_width), static_cast<int32_t>(m_height), 1 } } },
      .dstSubresource = {
        .aspectMask     = vk::ImageAspectFlagBits::eColor,
        .mipLevel       = 0,
        .baseArrayLayer = 0,
      ...
      .dstOffsets = { { vk::Offset3D{ 0, 0, 0 }, vk::Offset3D{ static_cast<int32_t>(m_width), static_cast<int32_t>(m_height), 1 } } },
    };

    VkImageBlit vkRegion = region;

    vkCmdBlitImage(a_cmdBuff, mainView.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, a_targetImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &vkRegion, VK_FILTER_NEAREST);
  }
